...
static void swap(struct _item *a, struct _item *b)
{
	struct _item tmp = *a;
	*a = *b;
	*b = tmp;
...
static void bubble_down(struct _pqueue *pq, size_t idx)
{
	while(2*idx + 1 < pq->size) {
		...
		if(right < pq->size && pq->data[right].priority < pq->data[to_swap].priority) {
			to_swap = right;
		}
...
static void bubble_up(pqueue *pq, size_t idx)
{
