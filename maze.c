    ...
    char *argv[])
{
    ...
    if (((flags & 4) == 4))
    {
        ...
        GraphSerializer_fromMaze(&network, mazeToSolve, &maxLineLength,
                                 &lineCount, flags);
    }
    ...
    else
    {
        ...
        network =
            GraphSerializer_fromFile(fp, &mazeToSolve, &maxLineLength,
                                     &lineCount, flags);
        if (!network)
        {
    ...
    size_t lineCount)
{
    ...
    size_t          tilesRemaining =
        ((maxLineLength - 1) * (lineCount - 1)) / 3;

    for (size_t i = tilesRemaining; i > 0; i--)
    {
        size_t          xCoord = 0;

        while (xCoord == 0 || xCoord >= maxLineLength - 1 ||
               xCoord >= lineCount - 1)
        {
        ...
        while (yCoord == 0 || yCoord >= maxLineLength - 1 ||
               yCoord >= lineCount - 1 || yCoord == xCoord)
        {
